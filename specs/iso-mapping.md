...
| **`payment:PaymentIntent`** | **PSD3 / Open Banking Standards** | Provides a standardized, machine-readable instruction for initiating M2M (machine-to-machine) payments. This aligns with the goals of Open Banking by enabling secure, automated, and auditable payment execution within a regulated ecosystem, where an agent's action (e.g., completing a task) can directly trigger a payment settlement. |
